...
const ColumnSelector = ({ columnDefinition, setColumnDefinition }) => {
  const adaptShowField = (headerToFind, value, colDef) =>
    colDef.reduce((acc, field) => {
  ...
  const adaptOrder = (headerToFind, value, colDef) =>
    colDef.reduce((acc, field) => {
      if (field.hasOwnProperty("columns")) {
      ...
      if (
        field.hasOwnProperty("Header") &&
        field.hasOwnProperty("order") &&
        field.Header === headerToFind
      ) {
        field.order = value;
      }
      return acc.concat(field);
  ...
  const handleShowChange = (Header, event) =>
    setColumnDefinition(
      adaptShowField(Header, event.target.checked, columnDefinition)
    );

  const handleOrderChange = (Header, event) =>
    setColumnDefinition(
      adaptOrder(Header, event.target.value, columnDefinition)
    );
      ...
      <tr key={field.Header}>
        ...
        <td>
          <input
            ...
            type="checkbox"
            checked={field.show}
            onChange={e => handleShowChange(field.Header, e)}
          />
        </td>
        {field.order ? (
          <td>
  ...
  return (
    <table>
      <tbody>
        {columnDefinition
          .filter(field => field.hasOwnProperty("Header"))
          .map(col => makeColumn(col))}
      </tbody>
    </table>
  );
};
